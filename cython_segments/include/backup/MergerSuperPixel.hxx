...
typedef struct LineBorder{
    ...
    int maxK;
    int size;
    LineBorder(int minV=1e9, int maxV=0, int s=0){
        minK = minV; maxK = maxV; size = s;
    }
}LineBD;

typedef struct BorderSeg{
    int x_h; // in H, x coord of the border pix on another side of the areal
    int y_v; // in V, y coord of the border pix on another side of the areal
...
typedef struct BorderInfo{
    ...
    void MergeBoundingBox(int *bbox1, int *bbox2){
    ...
    void AssignOneBorderSeg(Mkey_3D key, int oppos_xy, SegFitRst &seg_fit, bool is_row){
        if(is_row){
...
public:
    ...
    int         new_hist_dist[e_dist_num_ch*HIST_DIST_NUM_BIN];
    vector<int> new_hist_inst;
    double          new_inst_iou;
    
    // functions
    DistEdge(int s1=0, int s2=0, double edge=0):Edge(s1, s2, edge),border(){
        Reset();
    }
...
public:
    ...
    int         hist_dist[e_dist_num_ch*HIST_DIST_NUM_BIN];
    vector<int> hist_inst;
    double          inst_iou;
    string         svm_1st_str;
    string         svm_2nd_str;
    ...
    DistSuperPixel():Supix(),border(){
        ...
        memset(hist_dist, 0, e_dist_num_ch*HIST_DIST_NUM_BIN*sizeof(int));
    }
};
...
public:
    ...
    SuperPixelMerger(CDataTempl<double> *pSemMat, CDataTempl<double> *pDistMat, Segment_Stock *pSegStock, 
                        const GlbParam *pParam, CDataTempl<int> *pInstI=NULL):Graph(pSemMat->GetYDim(), pSemMat->GetXDim()){
        m_pSemMat            = pSemMat;
        m_pDistMat           = pDistMat;
    ...
    void ComputeEdgeWeights(int edge);

    // function working on adding member variables on Node and Edge.
    void  ComputeSuperPixelCost(int sup);
    double ComputeInstanceIoU(vector<int> &hist_inst, int det_size);
    ...
    double ComputeSemanticDifference(int sup0, int sup1);
    ...
    // svm classifier
    void   ReadClassifierFeatureNorm(string fname, int type);
    void   NormalizeSVMFeature(int num_node, int type=0);
    
    string SingleSuperPixelSVMFeature(int sup, int base_k, int &x_st, bool gen_str=true);
    string ConstructFinalClassifierFeature(DistEdge &edge, int &x_st, bool gen_str=true);
   
    string ConstructFinalClassifierFeature_inst(DistSuperPixel &supix, int &x_st, bool gen_str=true);
...
void SuperPixelMerger::GetMergeInfo(vector<double> &mergeInfo){
    ...
    for(auto it:m_supixs){
        ...
        for(auto it2:it.second.adjacents){
            ...
            vector<int> mrg_hist(m_num_instances, 0);
 ...
 */
void   SuperPixelMerger::ReadClassifierFeatureNorm(string fname, int type){
    int num_feature;
    string line;
    ifstream read;
    if(type == 0){
...
void SuperPixelMerger::NormalizeSVMFeature(int num_node, int type){
 ...
 */
string SuperPixelMerger::SingleSuperPixelSVMFeature(int sup, int base_k, int &x_st, bool gen_str){
    ...
    // histogram_distance
    for(int k =0; k < e_dist_num_ch*HIST_DIST_NUM_BIN; k++){
...
string SuperPixelMerger::ConstructFinalClassifierFeature(DistEdge &edge, int &x_st, bool gen_str){
 ...
 */
string SuperPixelMerger::ConstructFinalClassifierFeature_inst(DistSuperPixel &supix, int &x_st, bool gen_str){
    int cnt = x_st;
...
void SuperPixelMerger::UpdateSuperPixel(int sup, int edge){
    ...
    // instance count
    if(m_pParam->merge_gen_svm_train_en||m_pParam->merge_inst_svm_en || m_pParam->merge_merge_svm_en){
        memcpy(ref_supix.hist_dist, ref_edge.new_hist_dist, e_dist_num_ch*HIST_DIST_NUM_BIN*sizeof(int));
...
void SuperPixelMerger::ComputeEdgeWeights(int edge){
    auto ConstructNewRegionInfo = [&](DistEdge &ref_edge){
        ...
        for(int k=0; k < ref_edge.bd_pixs.size(); k ++){
            ...
            int py0 = (bd_pix.pix1) / m_wd;   
            int px0 = (bd_pix.pix1) % m_wd;
            int py1 = (bd_pix.pix2) / m_wd;   
            int px1 = (bd_pix.pix2) % m_wd;
...
void SuperPixelMerger::ComputeSuperPixelCost(int sup){
    auto FindOppositeX = [&](int py, int px, int step){
        int cx;
    ...
    auto FindOppositeY = [&](int py, int px, int step){
        int cy;
        if(false){
    ...
    for(int kk = 0; kk < e_dist_num_ch; kk ++){
...
double SuperPixelMerger::ComputeInstanceIoU(vector<int> &hist_inst, int det_size){
    double inst_iou = 0, cur_iou=0;
    for(int k=1; k < m_num_instances; k ++){
...
double SuperPixelMerger::ComputeSemanticDifference(int sup0, int sup1){
