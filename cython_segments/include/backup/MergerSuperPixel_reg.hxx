...
public:
    ...
    double      hist_w[e_dist_num_ch*HIST_W_NUM_BIN];
    UINT32     inst_id;
    string     svm_1st_str;
    string     svm_2nd_str;
    ...
    DistSuperPixel():Supix(),border(){
        ...
        bic_cost  = 0.0;
        perimeter = 0.0;
        memset(hist_w, 0, e_dist_num_ch*HIST_W_NUM_BIN*sizeof(double));
    }
};
...
public:
    SuperPixelMerger(CDataTempl<double> *pSemMat, Segment_Stock *pSegStock, const GlbParam *pParam, CDataTempl<UINT32> *pInstI=NULL):Graph(pSemMat->GetYDim(), pSemMat->GetXDim()){
        m_pSemMat    = pSemMat;
    ...
    double ComputeEdgeMergeCost(DistEdge &edge, double &merge_cost);
    ...
    double ComputeFitCost(UINT32 y0, UINT32 x0, UINT32 y1, UINT32 x1, UINT32 size);
    double ComputeBICcost(UINT32 numPix);
...
void SuperPixelMerger::GetDebugImage(CDataTempl<double> &debugI, UINT32 mode){
    for(UINT32 k=0; k < m_supixs.size(); k++){
        double val = m_supixs[k].fit_cost / m_supixs[k].pixs.size();
...
string SuperPixelMerger::ConstructFinalClassifierFeature(DistEdge &edge, UINT32 &x_st, bool gen_str){
    ...
    for(UINT32 k=0; k < m_num_sem; k++){
        double score_diff = m_supixs[edge.sup1].sem_score[k] - m_supixs[edge.sup2].sem_score[k];
        if (score_diff == 0)
            continue;
    ...
    if(m_pParam->merge_svm_en){
        ...
        for(int k=notIns_sups.size()-1; k>=0; k --){
            ...
            for(auto it2:ref_sup.adjacents){
                ...
                double merge_cost;
                double sem_diff = ComputeEdgeMergeCost(ref_edge, merge_cost);
...
double SuperPixelMerger::ComputeEdgeMergeCost(DistEdge &ref_edge, double &merge_cost){
    double sem_diff       = ComputeSemanticDifference(ref_edge.sup1, ref_edge.sup2);
    double sem_cost       = sem_diff >= m_pParam->merge_edge_semdiff_thr? m_pParam->merge_edge_semdiff_pnty : 0;
    ...
    double geo_cost         = ref_edge.new_perimeter/(supix0.pixs.size()-supix1.pixs.size());
    geo_cost              -= (supix0.perimeter/supix0.pixs.size() + supix1.perimeter/supix1.pixs.size());
    
    double merge_fit_cost = ref_edge.new_fit_cost - (supix0.fit_cost + supix1.fit_cost);
    double merge_bic_cost = ref_edge.new_bic_cost - (supix0.bic_cost + supix1.bic_cost);
    double fit_bic_cost   = merge_fit_cost + m_pParam->merge_edge_bic_alpha*merge_bic_cost;
    double connect_cost   = min(supix0.perimeter, supix1.perimeter)/double(ref_edge.bd_pixs.size());
...
void SuperPixelMerger::ComputeEdgeWeights(UINT32 edge){
    ...
    // compute merge cost.
    double sem_diff = ComputeEdgeMergeCost(ref_edge, ref_edge.mergecost);
...
void SuperPixelMerger::ComputeSuperPixelCost(UINT32 sup){
    ...
    UINT32 bbox_ht = ref_border.bbox[2]-y0 + 1;
    UINT32 bbox_wd = ref_border.bbox[3]-x0 + 1;
    double fit_cost = 0.0, bic_cost = 0.0;
    for(UINT32 k = 0; k < bbox_ht; k++){
...
double SuperPixelMerger::ComputeFitCost(UINT32 y0, UINT32 x0, UINT32 y1, UINT32 x1, UINT32 size){
    double fit_err  = m_pSegStock->GetAllSegFitErrorOnAny2Points(y0, x0, y1, x1);
    return (fit_err * size);
}

double SuperPixelMerger::ComputeBICcost(UINT32 numPix){
