...
cdef extern from "ProposalGeneratePy.hxx":
    ...
    cdef void ProposalGenerate(int *imgInfo, double *distVec, double *semVec, int *instVec, OutData *mydata);
            

@cython.boundscheck(False)
@cython.wraparound(False)
def py_DistPropsGenerate(np.ndarray[int, ndim=1, mode='c'] imgInfo not None, 
                         np.ndarray[double, ndim=1, mode='c'] distArr not None, 
                         np.ndarray[double, ndim=1, mode='c'] semArr not None, 
                         np.ndarray[int, ndim=1, mode='c'] instArr not None): 
